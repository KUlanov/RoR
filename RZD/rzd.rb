class Rzd
  ...
  def add_train(number_train, type_train, wagons)
    if type_train == 'Cargo'
      train_cargo_lists << TrainCargo.new(number_train, type_train, wagons)
    elsif type_train == 'Passenger'
      train_passenger_lists << TrainPassenger.new(number_train, type_train, wagons)
    end
  end
  ...
  def show_train_lists
    ...
    train_passenger_lists.each do |train|
      print "№#{train_passenger_lists.index(train) + 1} : #{train.number}."
    ...
    train_cargo_lists.each do |train|
      print "№#{train_cargo_lists.index(train) + 1} : #{train.number}"
      print "   Текущая станция: #{train.train_current_station.name}" if train.train_current_station
      puts " Количество вагонов: #{train.train_wagon_quantity}"
      train.show_wagon_list
      puts
    ...
    loop do
      ...
      when 3
        ...
        if current_route.find_station(station)
          ...
          puts 'Такая станция не существует в маршруте'
        end
      when 0
        break
      end
